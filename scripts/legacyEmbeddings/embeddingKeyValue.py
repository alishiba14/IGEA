...
def trainEmbeddings(input, records, noIds, noKeys, noVals, reverse_dictionary, epochs, embeddingsize, outDir, save=True):
    ...
    with graph.as_default():
        ...
        embeddings = tf.Variable(tf.random.uniform([input_layer_size, embeddingsize], -1.0, 1.0), name="embeddings")
        embed = tf.nn.embedding_lookup(embeddings, train_inputs)

        #softmax
        weights = tf.Variable(tf.random.truncated_normal([output_layer_size, embeddingsize],
...
def saveEmbeddings(input, embeddings, noIds, reverse_dictionary, epochs, embeddingsize,  outPath):
    fname  = outPath
...
if __name__ == "__main__":
    ...
    NUM_EPOCHS = config.getint('legacy', 'num_epochs')
    EMBEDDING_SIZE = config.getint('legacy', 'embedding_dim')
    INPUT_FILE = DATA_DIR + 'keyvals.tsv'
    OUTPUT_FILE = DATA_DIR + 'custom embeddings.csv'

    run(INPUT_FILE, NUM_EPOCHS, EMBEDDING_SIZE, OUTPUT_FILE)
